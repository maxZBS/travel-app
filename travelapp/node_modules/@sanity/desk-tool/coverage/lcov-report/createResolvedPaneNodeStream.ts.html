...
<head>
    <title>Code coverage report for createResolvedPaneNodeStream.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> createResolvedPaneNodeStream.ts</h1>
        <div class='clearfix'>
...
<a name='L392'></a><a href='#L392'>392</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import {Observable, of as observableOf, from as observableFrom, NEVER, concat} from 'rxjs'
import {switchMap, map, scan, distinctUntilChanged, pairwise, startWith, tap} from 'rxjs/operators'
...
&nbsp;
interface PaneResolutionErrorOptions {
  message: string
  context: RouterPaneSiblingContext
  helpUrlSlug?: string
  cause?: Error
...
class PaneResolutionError extends Error {
  ...
  context: RouterPaneSiblingContext
  helpUrlSlug: string | undefined
&nbsp;
  constructor({message, context, helpUrlSlug, cause}: PaneResolutionErrorOptions) {
    super(message)
    this.context = context
    this.helpUrlSlug = helpUrlSlug
    this.cause = cause
  }
}
&nbsp;
...
&nbsp;
    ...
    if (e instanceof PaneResolutionError) {
      ...
      console.warn(
        `Pane resolution error at index ${context.index}${
          context.splitIndex &gt; 0 ? <span class="branch-0 cbranch-no" title="branch not covered" >` for split pane index ${context.splitIndex}` </span>: ''
        }: ${e.message}${e.helpUrlSlug ? ` - see ${generateHelpUrl(e.helpUrlSlug)}` : <span class="branch-1 cbranch-no" title="branch not covered" >''}</span>`,
        e
      )
&nbsp;
...
export function createResolvedPaneNodeStream({
  ...
  initialCacheState = {
    ...
    resolvePane: <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >NEVER,</span>
  },
...
&nbsp;
      const wrapFn = (fn: PaneResolver): PaneResolver =&gt; {
        return (unresolvedPane, context, flatIndex) =&gt; {
...
&nbsp;
          try {
            result = fn(unresolvedPane, context, flatIndex)
          } catch (e) {
            if (e instanceof PaneResolutionError) {
              throw e
            }
&nbsp;
<span class="cstat-no" title="statement not covered" >            throw new PaneResolutionError({</span>
...
&nbsp;
      const resolvePane = wrapFn((unresolvedPane, context, flatIndex) =&gt; {
        if (!unresolvedPane) {
          throw new PaneResolutionError({
            message: 'Pane returned no child',
            context,
            helpUrlSlug: 'structure-item-returned-no-child',
          })
        }
&nbsp;
...
&nbsp;
          if (prevPane.routerPaneSibling.id === nextPane.routerPaneSibling.id) return prevPane
          return nextPane
        }),
      [] as ResolvedPaneMeta[]
    ),
...
&nbsp;
    ...
    startWith([] as ResolvedPaneMeta[]),
    pairwise(),
    map(([prev, curr]) =&gt;
      curr.map((currValue, index) =&gt; {
        const prevValue = prev[index] as ResolvedPaneMeta | undefined
        if (!prevValue) return currValue
&nbsp;
        if (hashResolvedPaneMeta(prevValue) === hashResolvedPaneMeta(currValue)) return prevValue
        return currValue
      })
    ),
    tap(console.log)
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Fri Oct 22 2021 01:52:05 GMT-0400 (Eastern Daylight Time)
            </div>
        </div>
