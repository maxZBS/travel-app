...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> helpers.ts</h1>
        <div class='clearfix'>
...
import {DocumentPaneNode, PaneNodeResolver, RouterPaneSiblingContext} from '../../types'
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof method !== 'function') {
...
&nbsp;
export const prevCurr = <span class="fstat-no" title="function not covered" >&lt;T</span>&gt;(
  initialValue: T
): OperatorFunction&lt;T, {prev: T | undefined; curr: T}&gt; =&gt; <span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >(s</span>ource) =&gt; {</span>
  const seed = <span class="cstat-no" title="statement not covered" >{prev: undefined, curr: initialValue}</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >  return scan&lt;T, {prev: T | undefined; curr: T}&gt;(</span>
<span class="fstat-no" title="function not covered" >    ({</span>curr}, next) =&gt; (<span class="cstat-no" title="statement not covered" >{prev: curr, curr: next})</span>,
    seed
  )(source)
}
&nbsp;
/**
 * takes in a `RouterPaneSiblingContext` and returns a string representation
 * that can be used inside of maps and keys
 */
...
type HashMetaKey =
  ...
  | 'paneNode'
&nbsp;
export const hashResolvedPaneMeta = (
  meta: ResolvedPaneMeta,
  options?: {omit: HashMetaKey[]}
): string =&gt; {
  const normalized: {[P in HashMetaKey]: unknown} = {
    type: meta.type,
...
&nbsp;
  return `metaHash(${JSON.stringify(omit(normalized, options?.omit || []))})`
}
&nbsp;
...
&nbsp;
// WeakMap require the first type param to extend `object`
// eslint-disable-next-line @typescript-eslint/ban-types
const randomIdCache = new WeakMap&lt;object, string&gt;()
&nbsp;
// this doesn't need to be secure but should have a decent amount of entropy
